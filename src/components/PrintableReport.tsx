    ...
    <>
      ...
      <style>{`
        @media print {
          ...
          .print-report .text-xs {
            font-size: 10px;
          }
          .print-report .text-sm {
            font-size: 11px;
          }
          .print-report .mb-4 {
            margin-bottom: 12px;
          ...
          .print-report .md\\:grid-cols-2 > div {
            display: block;
            margin-bottom: 8px;
          }
        }
      ...
      <div className="print-report bg-white text-black p-4 max-w-4xl mx-auto">
        ...
        {/* Psychotherapeutic Analysis - NEW SECTION */}
        {(() => {
          // Generate analysis on the fly if it doesn't exist
          const analysisData = patientProfile.psychotherapeuticAnalysis || 
                              patientProfile.specialistAnalysis || 
                              generateRealPsychotherapeuticAnalysis(patientProfile);
          ...
          return (
            <section className="mb-4 page-break-before">
              <h2 className="text-2xl font-bold mb-3 text-purple-700">
                Pre-Screeningowa Analiza Psychoterapeutyczna
              </h2>
              
              {/* Overall Summary */}
              <div className="bg-purple-50 border-2 border-purple-200 rounded-lg p-4 mb-4">
                <h3 className="text-lg font-semibold mb-2 text-purple-800">
                  Ogólna Gotowość: {analysisData.summary?.overallReadiness}
                </h3>
                
                {analysisData.summary?.keyStrengths && 
                 analysisData.summary.keyStrengths.length > 0 && (
                  <div className="mb-3">
                    <h4 className="font-semibold text-green-700 mb-1">Kluczowe Mocne Strony:</h4>
                    <ul className="list-disc list-inside space-y-1">
                      {analysisData.summary.keyStrengths.map((strength, idx) => (
                ...
                {analysisData.summary?.keyRisks && 
                 analysisData.summary.keyRisks.length > 0 && (
                  <div className="mb-3">
                    <h4 className="font-semibold text-red-700 mb-1">Kluczowe Ryzyka:</h4>
                    <ul className="list-disc list-inside space-y-1">
                      {analysisData.summary.keyRisks.map((risk, idx) => (
                ...
                {analysisData.summary?.therapeuticRecommendations && 
                 analysisData.summary.therapeuticRecommendations.length > 0 && (
                  <div>
                    <h4 className="font-semibold text-blue-700 mb-1">Rekomendacje Terapeutyczne:</h4>
                    <ul className="list-disc list-inside space-y-1">
                      {analysisData.summary.therapeuticRecommendations.map((rec, idx) => (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                {/* Psychotherapeutic Readiness Factors */}
                <div className="bg-gray-50 rounded-lg p-4">
                  <h3 className="text-lg font-semibold mb-3 text-gray-800">Gotowość Psychoterapeutyczna</h3>
                  <div className="space-y-3">
                    <div>
                      <span className="font-medium text-sm">Struktura osobowości:</span>
                      <span className="ml-2 text-sm text-gray-700">
                        {analysisData.psychotherapeuticReadiness?.personalityStructure?.value}
                      </span>
                    </div>
                    <div>
                      <span className="font-medium text-sm">Siła ego:</span>
                      <span className="ml-2 text-sm text-gray-700">
                        {analysisData.psychotherapeuticReadiness?.egoStrength?.value}
                      </span>
                    </div>
                    <div>
                      <span className="font-medium text-sm">Przymierze terapeutyczne:</span>
                      <span className="ml-2 text-sm text-gray-700">
                        {analysisData.psychotherapeuticReadiness?.therapeuticAlliance?.value}
                      </span>
                    </div>
                    <div>
                      <span className="font-medium text-sm">Zdolność integracji:</span>
                      <span className="ml-2 text-sm text-gray-700">
                        {analysisData.psychotherapeuticReadiness?.integrationCapacity?.value}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                {/* Set & Setting Factors */}
                <div className="bg-gray-50 rounded-lg p-4">
                  <h3 className="text-lg font-semibold mb-3 text-gray-800">Czynniki Set & Setting</h3>
                  <div className="space-y-3">
                    <div>
                      <span className="font-medium text-sm">Gotowość motywacyjna:</span>
                      <span className="ml-2 text-sm text-gray-700">
                        {analysisData.setSettingFactors?.motivationalReadiness?.value}
                      </span>
                    </div>
                    <div>
                      <span className="font-medium text-sm">Realizm oczekiwań:</span>
                      <span className="ml-2 text-sm text-gray-700">
                        {analysisData.setSettingFactors?.expectationRealism?.value}
                      </span>
                    </div>
                    <div>
                      <span className="font-medium text-sm">Zdolność do poddania się:</span>
                      <span className="ml-2 text-sm text-gray-700">
                        {analysisData.setSettingFactors?.surrenderCapacity?.value}
                      </span>
                    </div>
                    <div>
                      <span className="font-medium text-sm">System wsparcia:</span>
                      <span className="ml-2 text-sm text-gray-700">
                        {analysisData.setSettingFactors?.supportSystem?.value}
                      </span>
                    </div>
                  </div>
                </div>
              ...
              {/* Psychedelic-Specific Factors */}
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-4">
                <h3 className="text-lg font-semibold mb-3 text-yellow-800">Czynniki Specyficzne dla Terapii Psychodelicznej</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <div>
                      <span className="font-medium text-sm">Radzenie sobie ze stanami dysocjacyjnymi:</span>
                      <span className="ml-2 text-sm text-gray-700">
                        {analysisData.psychedelicFactors?.dissociativeExperienceHandling?.value}
                      </span>
                    </div>
                    <div>
                      <span className="font-medium text-sm">Komfort w zmienionych stanach:</span>
                      <span className="ml-2 text-sm text-gray-700">
                        {analysisData.psychedelicFactors?.altereredStatesComfort?.value}
                      </span>
                    </div>
                    <div>
                      <span className="font-medium text-sm">Rezygnacja z kontroli:</span>
                      <span className="ml-2 text-sm text-gray-700">
                        {analysisData.psychedelicFactors?.controlRelinquishing?.value}
                      </span>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div>
                      <span className="font-medium text-sm">Umiejętności mindfulness:</span>
                      <span className="ml-2 text-sm text-gray-700">
                        {analysisData.psychedelicFactors?.mindfulnessSkills?.value}
                      </span>
                    </div>
                    <div>
                      <span className="font-medium text-sm">Poprzednie doświadczenia psychodeliczne:</span>
                      <span className="ml-2 text-sm text-gray-700">
                        {analysisData.psychedelicFactors?.previousPsychedelicExperience?.value}
                      </span>
                    </div>
                  </div>
                </div>
              ...
              {analysisData.narrativeAssessment && (
                <div className="bg-white border border-gray-200 rounded-lg p-4">
                  <h3 className="text-lg font-semibold mb-3 text-gray-800">Szczegółowa Ocena Narracyjna</h3>
                  
                  {analysisData.narrativeAssessment.personalityDynamics && (
                    <div className="mb-3">
                      <h4 className="font-semibold text-sm text-gray-700 mb-1">Dynamika Osobowości:</h4>
                  ...
                  {analysisData.narrativeAssessment.traumaAndDefenses && (
                    <div className="mb-3">
                      <h4 className="font-semibold text-sm text-gray-700 mb-1">Trauma i Mechanizmy Obronne:</h4>
                      <div className="text-sm text-gray-600 whitespace-pre-wrap">
                        {analysisData.narrativeAssessment.traumaAndDefenses}
                      </div>
                    </div>
                  ...
                  {analysisData.narrativeAssessment.therapeuticRelationship && (
                    <div className="mb-3">
                      <h4 className="font-semibold text-sm text-gray-700 mb-1">Relacja Terapeutyczna:</h4>
                      <div className="text-sm text-gray-600 whitespace-pre-wrap">
                        {analysisData.narrativeAssessment.therapeuticRelationship}
                      </div>
                    </div>
                  ...
                  {analysisData.narrativeAssessment.readinessAssessment && (
                    <div className="mb-3">
                      <h4 className="font-semibold text-sm text-gray-700 mb-1">Ocena Gotowości:</h4>
                      <div className="text-sm text-gray-600 whitespace-pre-wrap">
                        {analysisData.narrativeAssessment.readinessAssessment}
                      </div>
                    </div>
                  ...
                  {analysisData.narrativeAssessment.riskMitigation && (
                    <div className="mb-3">
                      <h4 className="font-semibold text-sm text-gray-700 mb-1">Mitygacja Ryzyka:</h4>
                      <div className="text-sm text-gray-600 whitespace-pre-wrap">
                        {analysisData.narrativeAssessment.riskMitigation}
                      </div>
                    </div>
                  ...
                  {analysisData.narrativeAssessment.preparationPlan && (
                    <div>
                      <h4 className="font-semibold text-sm text-gray-700 mb-1">Plan Przygotowania:</h4>
                      <div className="text-sm text-gray-600 whitespace-pre-wrap">
                        {analysisData.narrativeAssessment.preparationPlan}
                      </div>
                    </div>
